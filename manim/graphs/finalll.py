...
f1 = lambda x, y: y - np.sin(x)
f2 = lambda x, y: y - np.sin(2*x)
f3 = lambda x, y: y - np.sin(2*x) - 0.5
f4 = lambda x, y: y - abs(np.sin(2*x)+0.5)
f5 = lambda x, y: abs(y) - abs(np.sin(2*x) + 0.5)
equations = [f1, f2, f3, f4, f5]
tex_equations = ["$y=\sin(x)$", "$y=\sin(2x)$", "$y=\sin(2x) + 0.5$", "$y = |\sin(2x) +0.5|$", "$|y|=|\sin(2x)+0.5|$"]
