...
f1 = lambda x, y: y - (x**2 -2*x -3)
f2 = lambda x, y: y - (abs(x)**2-2*abs(x) -3)
f3 = lambda x, y: y - abs(abs(x)**2 -2*abs(x)-3)
f4 = lambda x, y: abs(y) - abs(abs(x)**2 -2*abs(x)-3)
equations = [f1, f2, f3, f4]
tex_equations = ["$y=x^2-2x-3$", "$y=|x|^2-2|x|-3$", "$y = ||x|^2-2|x|-3|$", "$|y| = ||x|^2-2|x|-3|$"]
